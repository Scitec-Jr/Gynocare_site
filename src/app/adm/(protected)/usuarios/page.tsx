...
const columns: TableColumn<User>[] = [
	...
	},
	{
		key: "status",
		label: "Status",
		render: (value) => <span className={`px-3 py-1 rounded text-sm font-semibold ${value ? "bg-green-100 text-green-700" : "bg-red-100 text-red-700"}`}>{value ? "Ativo" : "Inativo"}</span>,
	},
	{
		key: "createdAt",
		label: "Criado em",
...
export default function UsersPage() {
	...
	return (
		<div>
			...
			<AdminTable<User> columns={columns} data={paginatedUsers} onEdit={openEditModal} onDelete={openDeleteModal} />

			{totalPages > 1 && <Pagination currentPage={currentPage} totalPages={totalPages} onPageChange={setCurrentPage} />}

			{/* Create/Edit Modal */}
			<AdminModal isOpen={modal.isOpen && (modal.type === "create" || modal.type === "edit")} title={modal.type === "create" ? "Novo Usuário" : "Editar Usuário"} onClose={() => setModal({ isOpen: false, type: "create", data: null })} onConfirm={handleSubmit} confirmText={modal.type === "create" ? "Criar" : "Salvar"}>
				<div className="space-y-4">
					<FormField label="Nome Completo" name="name" value={formData.name} onChange={(value) => setFormData({ ...formData, name: String(value) })} required placeholder="Ex: João Silva" />
					<FormField label="Email" name="email" type="email" value={formData.email} onChange={(value) => setFormData({ ...formData, email: String(value) })} required placeholder="Ex: joao@gynocare.com.br" />
					<FormField
						label="Função"
						name="role"
						type="select"
