...
export default function ProceduresPage() {
	...
	const filteredProcedures = useMemo(() => {
		return procedures.filter((proc) => proc.name.toLowerCase().includes(searchTerm.toLowerCase()) || proc.slug.toLowerCase().includes(searchTerm.toLowerCase()));
	...
	return (
		<div>
			...
			<AdminTable<Procedure> columns={columns} data={paginatedProcedures} onEdit={openEditModal} onDelete={openDeleteModal} />

			{totalPages > 1 && <Pagination currentPage={currentPage} totalPages={totalPages} onPageChange={setCurrentPage} />}

			{/* Create/Edit Modal */}
			<AdminModal isOpen={modal.isOpen && (modal.type === "create" || modal.type === "edit")} title={modal.type === "create" ? "Novo Procedimento" : "Editar Procedimento"} onClose={() => setModal({ isOpen: false, type: "create", data: null })} onConfirm={handleSubmit} confirmText={modal.type === "create" ? "Criar" : "Salvar"}>
				<div className="space-y-4">
					<FormField label="Nome" name="name" value={formData.name} onChange={(value) => setFormData({ ...formData, name: String(value) })} required placeholder="Ex: Ginecologia" />
					<FormField label="Slug" name="slug" value={formData.slug} onChange={(value) => setFormData({ ...formData, slug: String(value) })} required placeholder="Ex: ginecologia" />
				</div>
			</AdminModal>
