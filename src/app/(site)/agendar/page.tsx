...
type Procedimento = { id: number; name: string };
type Exame = { id: number; name: string };
type Doutor = { id: number; name: string };
...
export default function Agendamento() {
	...
	const [examId, setExameId] = useState<number | null>(null);
	const [doctorId, setDoutorId] = useState<number | null>(null);
	...
	useEffect(() => {
		fetch("/api/procedimentos")
			.then((res) => res.json())
			.then((data) => setProcedimentos(data.data));
	}, []);
	...
	useEffect(() => {
		...
		fetch(`/api/doutores?exameId=${examId}`, {
			signal: controller.signal,
		})
			.then((res) => res.json())
			.then((data) => {
	...
	useEffect(() => {
		if (!doctorId || !examId) return;

		const controller = new AbortController();

		fetch(`/api/disponibilidade?doutorId=${doctorId}&exameId=${examId}`, {
			signal: controller.signal,
		})
			.then((res) => res.json())
	...
	const startDate = new Date(`2000-01-01T${horarioSelecionado}:00`);
	...
	const confirmarAgendamento = async () => {
		if (!doctorId || !examId || !dataSelecionada || !horarioSelecionado || !phone) return;
		...
		try {
			...
			const nomeExame = exames.find((e) => e.id === examId)?.name || "";
			const nomeProcedimento = procedimentos.find((p) => p.id === procedimentoId)?.name || "";
			const nomeMedico = doutores.find((d) => d.id === doctorId)?.name || "";
		...
		<main className="min-h-screen bg-linear-to-br from-slate-50 to-slate-100 py-12 px-4">
			<section className="max-w-4xl mx-auto mb-8">
				...
				<div className="space-y-6">
					...
					{doctorId && examId && datas.length > 0 && (
						...
						<div className="bg-linear-to-r from-(--main-color)/10 to-(--main-color)/5 rounded-lg p-6 border-2 border-(--main-color) animate-in fade-in duration-300">
							...
							<div className="space-y-3 text-sm text-slate-700 mb-6">
								<div className="flex justify-between bg-white bg-opacity-50 p-3 rounded">
									...
									<span>{procedimentos.find((p) => p.id === procedimentoId)?.name}</span>
								</div>
								<div className="flex justify-between bg-white bg-opacity-50 p-3 rounded">
									<span className="font-medium">Exame:</span>
									<span>{exames.find((e) => e.id === examId)?.name}</span>
								</div>
								<div className="flex justify-between bg-white bg-opacity-50 p-3 rounded">
									<span className="font-medium">Médico:</span>
									<span>{doutores.find((d) => d.id === doctorId)?.name}</span>
								</div>
								<div className="flex justify-between bg-white bg-opacity-50 p-3 rounded">
							...
							<div className="mb-4">
								...
								<input type="tel" placeholder="Digite o seu telefone, apenas números" value={phone} onChange={(e) => setTelefone(e.target.value.replace(/\D/g, ""))} className="w-full px-4 py-3 border-2 border-slate-200 rounded-lg focus:border-(--main-color) focus:outline-none focus:ring-2 focus:ring-(--main-color)/10 transition-all" maxLength={11} />
							</div>
							<button onClick={confirmarAgendamento} disabled={!phone} className="w-full bg-(--main-color) text-white font-semibold py-3 rounded-lg hover:opacity-90 transition-all shadow-md hover:shadow-lg disabled:opacity-50 disabled:cursor-not-allowed">
			...
			<section className="max-w-4xl mx-auto">
				<p className="font-bold">
					Prezado paciente, <br />
					<br />
					Gostaríamos de informar que, para garantir a eficiência e organização do nosso atendimento, pedimos que você confirme sua consulta/exame com antecedência mínima de 12 horas. <br />
					<br />
